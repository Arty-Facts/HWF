...
userWss.on("connection", (ws, req) => {
    ...
    ws.on("message", (message) => {
        ...
        console.log("=============================\n");
        //TODO: Do load balancing here and choose the right agent
        // let agent = getBestAgent(message)
        // if (agent.idle) { sendToAgent(message, agent.ws) }
        // else { queueTask(message)}
        let agent = agents[0];
...
function sendToAgent(data, agent) {
    if (agent) {
        try {
